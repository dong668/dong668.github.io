...
$(document).ready(function() {
	...
	if (!null) {
		$('nav ul').jarvismenu({
			accordion: true,
			speed: $.menu_speed,
			closedSign: '<em class="icon icon-expand-o"></em>',
			openedSign: '<em class="icon icon-collapse-o"></em>'
		});
	} else {
	...
	$('button[data-loading-text]').on('click',
		function() {
			var btn = $(this) btn.button('loading') setTimeout(function() {
					btn.button('reset')
				},
				3000)
		});
	...
	$('#refresh').click(function(e) {
		$.SmartMessageBox({
				title: "<i class='icon icon-refresh' style='color:green'></i> Clear Local Storage",
	...
	$('#logout a').click(function(e) {
		...
		// ask verification
		$.SmartMessageBox({
				title: "<i class='icon icon-sign-out txt-color-orangeDark'></i> Logout <span class='txt-color-orangeDark'><strong>" + $('#show-shortcut').text() + "</strong></span> ?",
...
function runAllForms() {
	...
	$("form").submit(function(e) {
		...
		if (typeof(__checkDataValid) != 'undefined') {
			if (!__checkDataValid()) {
				return false;
			}
		}
		...
		$(this).find(":input").each(function(e) {
			...
			if (name && name.length > 0) {
				...
				if (type == 'radio') {
					post[name] = $(_this).find('input[name="' + name + '"]:checked').val();
				}
				if (type == 'checkbox') {
					var v = [];
					$(_this).find("input[name='" + name + "']:checked").each(function() {
						v.push($(this).val());
					});
					post[name] = v;
				}
		...
		$.ajax({
			...
			beforeSend: function() {
				// cog placed
				container.html('<h1><i class="icon icon-cog icon-spin"></i> Submitting...</h1>');
			...
			error: function(xhr, ajaxOptions, thrownError) {
				container.html('<h4 style="margin-top:10px; display:block; text-align:left"><i class="icon icon-warning txt-color-orangeDark"></i> Error 404! Page not found.</h4>');
...
function setup_widgets_desktop() {
	...
	if ($.fn.jarvisWidgets && $.enableJarvisWidgets) {
		...
		$('#widget-grid').jarvisWidgets({
			...
			// toggle button
			toggleButton: true,
			toggleClass: 'icon icon-minus | icon icon-plus',
			toggleSpeed: 200,
			onToggle: function() {},
			// delete btn
			deleteButton: true,
			deleteClass: 'icon icon-remove',
			deleteSpeed: 200,
			onDelete: function() {},
			// edit btn
			editButton: true,
			editPlaceholder: '.jarviswidget-editbox',
			editClass: 'icon icon-cog | icon icon-save',
			editSpeed: 200,
			onEdit: function() {},
			// color button
			colorButton: true,
			...
			fullscreenClass: 'icon icon-resize-full | icon icon-resize-small',
			fullscreenDiff: 3,
			...
			refreshButton: true,
			refreshButtonClass: 'icon icon-refresh',
...
function loadURL(url, container) {
	$.ajax({
		...
		dataType: 'html',
		cache: true,
		// (warning: this will cause a timestamp and will call the request twice)
		beforeSend: function() {
			// cog placed
			container.html('<h1><i class="icon icon-cog icon-spin"></i> Loading...</h1>');
		...
		error: function(xhr, ajaxOptions, thrownError) {
			container.html('<h4 style="margin-top:10px; display:block; text-align:left"><i class="icon icon-warning txt-color-orangeDark"></i> Error 404! Page not found.</h4>');
			$.getJSON('/manager/users/logged_in.json',
				function(json) {
					if (!json.rt) {
						window.location.href = '/manager/users/login';
					}
...
function jsonLoad(url, params, callback) {
	params = params || {};
...
function drawBreadCrumb() {
	...
	$("#ribbon ol.breadcrumb").append($("<li><a href='#'>首页</a></li>"));
...
function pageSetUp() {
	...
	$("img[tolerant-error!=1]").error().attr('src', 'http://pic.mimixiche.com/noimg.gif');
}
